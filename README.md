...
> 只在VPS上安装derper，没有安装Tailscale。官方安装说明中虽然有Tailscale客户端一起安装，其实没有意义。使用角度上来说国内不用VPS加入节点，不用SSH到VPS。不用国内服务器做内网代理以及出口代理。
>   
>
...
Cloudflare
进入 “My Profile”——API Tokens → Create Token——模板：Zone / DNS Edit——最小权限原则——复制 Token 填入脚本

阿里云：
RAM → 创建子用户——添加 "AliDNSFullAccess"——创建 AccessKey

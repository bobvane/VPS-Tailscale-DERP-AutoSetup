#!/bin/bash
# ============================================================
# VPS-Tailscale-DERP-AutoSetup 中国优化版 v4.3-pro-derpmapfix-full
# 作者: bobvane
# 功能: 自动部署 DERP + BBR + 证书 + Tailscale + 管理菜单
# 仅修复: derpmap.json 未生成问题 (路径不存在 / 写入失败)
# ============================================================
...
if certbot certonly --standalone -d "$DOMAIN" --preferred-challenges http \
  ...
  error "❌ 证书申请失败，请检查 80 端口与 DNS 解析。"
  exit 1
fi
...
if [ -s /etc/tailscale/derpmap.json ]; then
  info "✅ 已生成 derpmap.json 并保存到 /etc/tailscale/derpmap.json"
else
  warn "⚠️ derpmap.json 写入失败，请手动检查 /etc/tailscale/ 目录权限"
fi

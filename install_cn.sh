...
# =====================================================
# 7) 安装完成 — 部署 td 管理工具、创建续签任务、重启提示
# =====================================================

info "部署 DERP 管理工具 td..."
...
if [[ ! -f "${SCRIPT_DIR}/td" ]]; then
  error "未找到 ${SCRIPT_DIR}/td 文件，无法继续。请确保仓库完整。"
  exit 1
fi
...
info "td 工具已安装到 /usr/local/bin/td"
echo

# -----------------------------------------------------
# 创建自动续签任务（每天 2 次）
# certbot renew --quiet 会在证书未到期时自动退出
# derper 会在证书更新时自动重启（deploy-hook），
# 但这里依然添加 cron 作为增强保障机制。
# -----------------------------------------------------

CRON_FILE="/etc/cron.d/derper-auto-renew"
info "创建自动续签计划任务: ${CRON_FILE}"
...
info "自动续签任务已创建（每 12 小时执行一次 certbot renew）"
echo
...
echo
echo -e "${GREEN}安装流程已全部完成！${RESET}"
echo


# -----------------------------------------------------
# 由于启用了新内核 + BBR，需要重启后才会真正生效
# -----------------------------------------------------
